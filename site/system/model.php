...
class model {
    ...
    public function readLivre($expressao) {
        $q = $this->db->prepare($expressao);
        $q->execute();
        $q->setFetchMode(PDO::FETCH_ASSOC);
        $data = $q->fetchAll();
        $this->count = count($data);
    ...
    public function updateLivre($expressao) {
        $sql = $expressao;
        $exec = true;
        // echo $sql."<br>";die;
...
class gestor {
    ...
    protected $db;
    protected $host = DBHOST_GESTOR;
    protected $database = DBNAME_GESTOR;
    protected $user = DBUSER_GESTOR;
    protected $password = DBPASS_GESTOR;
    protected $last_query;
    protected $count;
    public $_table;
    protected $_sql;
    ...
    public function read($table = NULL, $fields = NULL, $where = NULL, $group_by = NULL, $limit = NULL, $offset = NULL, $orderby = NULL, $freeQuerie = NULL) {
        ...
        $offset = ($offset != NULL ? "OFFSET {$offset}" : NULL);
        $orderby = ($orderby != NULL ? "ORDER BY {$orderby}" : NULL);
        $sql = "SELECT SQL_CALC_FOUND_ROWS {$fields} FROM {$this->_table} {$where} {$group_by} {$orderby} {$limit} {$offset} {$freeQuerie}";
        //echo $sql.'<hr />'; die;
