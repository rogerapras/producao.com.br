    ...
    public function montarPaginacao($array, $range = 10) {
        ...
        for ($i = $array['page_start']; $i < ($array['page_end'] + 1); $i++) {
            ...
            if ($i == $array['pagina_atual']) {
                ...
                $lista[] = "<li><a title=\"Página $i\" href=\"" . $url . "/" . $i . "\">$i</a></li>";
            }
        }

        $lista[] = ($array['go_next']) ? '<li><a href="' . $url . '/' . ($pagina_atual_explode + 1) . '">Próxima</a></li>' : null;
        $lista[] = ($array['go_last']) ? '<li><a href="' . $url . '/' . $array['total_pages'] . '">Última</a></li>' : null;
        ...
        if ($array['pagina_atual'] <= $array['total_pages']) {
            $menu .= "<p class=\"clear\">Exibindo página <strong>" . $array['pagina_atual'] . "</strong> de <strong>" . $array['total_pages'] . "</strong> em um total de <strong>" . $array['total_regs'] . "</strong> registro(s).";
        }
