...
class insumoModel extends model {
    ...
    public function getInsumoPP($where = null) {
        ...
        $tables .= ' left join prodMarca as m on m.idMarca = a.idMarca';
        $tables .= ' left join prodModelo as c on c.idModelo = a.idModelo';
        $tables .= ' left join prodFornecedor as f on f.idFornecedor = a.idFornecedorUltimaCompra';
        $orderby = 'a.dsInsumo';
        return $this->read($tables, array('a.*', 'd.dsGrupo', 'm.dsMarca', 'c.dsModelo', 'f.dsFornecedor'), $where, null, null, null, $orderby);
    }
    //Grava o perfil
