...
class osanalise extends controller {
    ...
    public function index_action() {
        ...
        $this->smarty->display('osanalise/lista.html');
        
    }

//Funcao de Busca
    public function busca_osanalise() {
    ...
    //Funcao de Inserir
    public function novo_osanalise() {
        ...
        $modelStatus = new statusosModel();
        $lista_statusOS = array('' => 'SELECIONE');
        foreach ($modelStatus->getStatusOS('idStatusOS = 4 or idStatusOS = 5 or idStatusOS = 6') as $value) {
            $lista_statusOS[$value['idStatusOS']] = $value['dsStatusOS'];
        }
        
        $this->smarty->assign('registro', $registro);
        $this->smarty->assign('idOS', $idOS);
        $this->smarty->assign('lista_statusOS', $lista_statusOS);        
        ...
        $this->smarty->assign('title', 'Ordem de Serviço');
        $this->smarty->display('osanalise/form_novo.tpl');
    }
    // Gravar Padrao
    public function gravar_os() {
        ...
        $datastatus = array(
            ...
            'dsObservacao' => 'Digitação da Analise/Causa',
    ...
    private function trataPost($post) {
        ...
        $data['idStatusOS'] = ($post['idStatusOS'] != '') ? $post['idStatusOS'] : 1;
        ...
        $data['dsAnaliseCausa'] = ($post['dsAnaliseCausa'] != '') ? $post['dsAnaliseCausa'] : null;
